...
class FormTestCase(TestCase):
    """
    TODO: add manual form and test generate_safe_form
    """
    def setUp(self):
        c1 = Category.objects.create(name='Default')
        c2 = Category.objects.create(name='limit2', display_all=False, display_factor=2)
        q1 = Question.objects.create(question='Test1',
                                     is_active=True,
                                     category=c1,
                                     required=True
                                     )
        q2 = Question.objects.create(question='Test2',
                                     is_active=True,
                                     category=c2,
                                     required=True
                                     )
        Question.objects.create(question='Test3',
                                     is_active=True,
                                     category=c2,
                                     required=True
                                     )
        Question.objects.create(question='Test4',
                                     is_active=True,
                                     category=c2,
                                     required=True
                                     )
    ...
    def test_form_creation(self):
        """
        This will throw an exception if the code in .tools.helper.generate_safe_forms incorrect
        :return:
        """
        client = Client()
    ...
    def test_question_limitation(self):
        """
        test generator for form questions
        pre_def = limited questions list obj
        questions = generated question list obj
        :return:
        """
        ques = Question.objects.filter(is_active=True).order_by('count')
        questions = get_questions()
        questions = list(chain(questions))
        pre_def = list(chain(ques.filter(question='Test1'),
                             ques.filter(question='Test2'),
                             ques.filter(question='Test3')))
        self.assertEqual(questions, pre_def, msg='Generator error')

    def test_json_response(self):
        client = Client()
        response = client.get(reverse('get_data'))
        self.assertEqual(response.status_code, 200,  msg='Request error get_data')

    def test_empty_json_response(self):
        """
        test for empty get request.
        ...
        x = response.get('_x')
        y = response.get('_y')
        self.assertEqual(question, None, msg='Response error JSON obj question wrong')
    ...
    def test_filled_json_response(self):
        """
        test for request with a question and a single answer
        :return:
        """
        client = Client()
        question = Question.objects.get(question='Test1')
        response = client.get(path=reverse('get_data'), data={'id': question.pk})
        ua = UserAnswer.objects.filter(question=question)
        x = list()
        y = list()
        for t in ua:
            x.append(t.answer)
            y.append(str(round(ua.filter(answer=t).count() / ua.count(), ndigits=2) * 100))
        data = {
            '_x': x,
            '_y': y,
        ...
        self.assertJSONEqual(
            ...
            expected_data=data,
            msg='Response error get_data wrong created JSON obj'
        )
...
class PollTestCase(TestCase):
    def setUp(self):
        ...
        q2 = Question.objects.create(question='Test2',
                                     ...
                                     category=c,
                                     required=True,
                                     type=Question.QUESTION_TYPE_SINGLE,
                                     )
        q3 = Question.objects.create(question='Test3',
                                     is_active=True,
                                     category=c,
                                     required=False
                                     )

...
class QuestionAdmin(admin.ModelAdmin):
    ...
    fields = ('question', 'description', 'is_active', 'answers', 'type',
        'category', 'number_range_end', 'count')
    readonly_fields = 'count',

    def get_obj_url(self, obj):
        return format_html('<a href="{0}">Edit</a>', reverse(
            'admin:poll_question_change', args=(obj.pk,), current_app='poll'))
    get_obj_url.allow_tags = True
    get_obj_url.short_description = _('Actions')


class UserAnswerAdmin(admin.ModelAdmin):
    list_display = 'answer', 'type', 'ip', 'domain'
